...
// Translate an AI/provider error into a safe, user-friendly message.
// We never surface provider internals, stack traces or raw error text to the client.
const friendlyAIError = (err) => {
    const status = err?.status;
    if (status === 429) return 'The AI service is busy right now. Please wait a moment and try again.';
    if (status === 403) return 'The AI service is currently unavailable due to an access restriction on the configured API key. Please contact the site administrator.';
    if (status === 404) return 'The configured AI model is unavailable. Please contact the site administrator.';
    if (status === 400) return 'The AI service could not process this request. Please try rephrasing your question.';
...
export const summarizePDF = async (req, res) => {
    ...
    try {
        ...
        console.log(`[summarizePDF] Sending prompt to Groq AI for PDF: ${pdf._id}`);
        ...
        try {
            ...
            res.status(502).json({
                success: false,
                message: friendlyAIError(aiError)
            });
        }
    } catch (error) {
...
export const generatePDFFlow = async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const flow = result.choices[0].message.content;
