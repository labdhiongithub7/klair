...
export const chunkText = (text, maxChunkSize = 2000, overlap = 200) => {
    ...
    while (startIndex < text.length) {
        ...
        // Stop once the whole text has been consumed. Without this, the overlap
        // rewind below re-emits the tail of the document forever, advancing one
        // character per iteration and producing hundreds of near-identical chunks.
        if (endIndex >= text.length) {
            break;
        }

        // Move start index forward, accounting for overlap
        const nextStartIndex = Math.max(endIndex - overlap, startIndex + 1);

        // Prevent infinite loop if we failed to make forward progress
