...
const useChatStore = create((set, get) => ({
  ...
  askQuestion: async (question, userId) => {
    ...
    try {
      ...
      // Remove the optimistic question bubble so we don't leave a dangling,
      // unanswered message in the thread.
      set(state => ({
        messages: state.messages.filter(m => m !== userMessage)
      }));

      // Surface a clear error instead of silently doing nothing.
      const serverMessage = error.response?.data?.message;
      toast.error(serverMessage || 'Failed to get an answer. Please try again.', {
        duration: 5000,
        icon: '❌',
      });
    } finally {
